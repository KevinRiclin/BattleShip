...
export default function ProfessionalBackground() {
  ...
  const pathRefsRef = useRef<SVGPathElement[]>([]);
  ...
  useEffect(() => {
    ...
    if (!sphereEl || !containerEl) return;

    let resizeTimeout: NodeJS.Timeout;

    const fitElementToParent = () => {
      const parentEl = containerEl.parentNode as HTMLElement;
      const elOffsetWidth = sphereEl.getBoundingClientRect().width;
      const parentOffsetWidth = parentEl?.offsetWidth || window.innerWidth;
      const ratio = Math.min(parentOffsetWidth / elOffsetWidth, 1);
      setScale(ratio);
    };
    ...
    const handleResize = () => {
      ...
      resizeTimeout = setTimeout(fitElementToParent, 150);
    };
    
    window.addEventListener("resize", handleResize);

    const pathEls = sphereEl.querySelectorAll("path");
    pathRefsRef.current = Array.from(pathEls);

    const startTime = performance.now();

    const breathAnimate = (currentTime: number) => {
      const time = currentTime - startTime;

      pathEls.forEach((path, i) => {
        const percent = (1 - Math.sin(i * 0.35 + time * 0.0022)) / 2;

        const colorPercent = percent;
        const r = Math.round(17 * (1 - colorPercent) + 80 * colorPercent);
        const g = Math.round(69 * (1 - colorPercent) + 80 * colorPercent);
        const b = Math.round(198 * (1 - colorPercent) + 80 * colorPercent);
        const a = (1 - colorPercent) * 1 + colorPercent * 0.35;

        (path as SVGPathElement).style.stroke = `rgba(${r}, ${g}, ${b}, ${a})`;

        const translatePercent = percent;
        const translateX = 2 + (translatePercent - 1) * 6;
        const translateY = 2 + (translatePercent - 1) * 6;
        (path as SVGPathElement).style.transform = `translate(${translateX}px, ${translateY}px)`;
      });

      animationIdRef.current = requestAnimationFrame(breathAnimate);
    };
  ...
  return (
    <div className="fixed inset-0 z-0 bg-black flex justify-center items-center overflow-hidden">
      <Particles
      ...
      <div
        ref={containerRef}
        className="w-1/2 aspect-square relative z-10"
        style={{
          paddingBottom: "50%",
        }}
      >
